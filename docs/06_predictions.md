...
スペクトル次元 $d_s$ は拡散（熱核）に基づく量であり、本理論の $n + 1$ とは定義が異なる。両端値の一致（ $2$ と $4$ ）は確定した対応だが、定義間の同値性は未証明であり予想の地位に留まる。また系9.7.2の換算は次元が変化する領域に $1/4$ 乗則を外挿しており、遷移域の詳細形状は近似である。LQG・スピンフォームの $d_s$ 計算は状態依存性の議論が継続中である。
