...
観測空間の代数的構造に以下の2つの要請を課す。

要請O1（可逆性）：物理的測定の合成は結合則を満たし、かつ逆演算を持つ。

すなわち測定 $A, B$ の合成 $(A \circ B)$ に対して $(A \circ B)^{-1} = B^{-1} \circ A^{-1}$ が成立しなければならない。この等式は $(AB)(B^{-1}A^{-1}) = A(BB^{-1})A^{-1} = 1$ という展開に結合則 $(AB)C = A(BC)$ を必要とする。したがって**非結合的代数は要請O1を直接破る**。

要請O2（立体的観測可能性）：観測可能な空間次元は、独立な3次元回転群 $SO(3)$ 以上の回転対称性が存在する最小次元でなければならない。$SO(2)$（1軸回転）では観測者は「面の内部」にとどまり、独立した複数方向の奥行きを区別できない。立体的な空間の認識には最低 $SO(3)$ が必要である。

### 4.2 Hurwitz定理による導出

要請O1の結合則より、観測空間の代数は結合的除算代数でなければならない。
...
要請O1・O2を同時に満たす唯一の除算代数は四元数 $\mathbb{H}$ であり、その虚数部の次元が観測空間の次元を与える：
...
基本方程式における $n^\ast = 11$ は §5 で観測値を用いず独立に確定している。したがって基本方程式は $\alpha$ の**定義式**であり、$n^\ast$ を決める方程式ではない。この点は明示しておく。

基本方程式が**独立な物理的制約**として機能することは、以下の整合性チェックで確認される。$\alpha$ を基本方程式とは完全に独立な経路、すなわち $\Lambda$ の観測値 $\Lambda_{\rm obs} = 1.089 \times 10^{-52}\,{\rm m}^{-2}$ と $n^\ast = 11$ のみから逆算すると：

$$\alpha_\Lambda = -\frac{\ln\!\left(\dfrac{\Lambda_{\rm obs}\, l_P^2}{2(n^\ast-2)}\right)}{\ln n^\ast} = 117.920$$

基本方程式が与える $\alpha_\star = 117.913$ との差は $0.006\%$ である。この2つの独立な経路が同じ $\alpha$ を与えることは**偶然ではなく**、理論の内部整合性を示す。もし基本方程式が単なる恒等式であれば、この一致は意味を持たない。
...
係数 $2(n^\ast-2)$ は基本方程式の**連続する整数点での差分**から代数的必然として導かれる。
...
単一の公理（公理Ω）から出発し、確立された外部数学定理（Hurwitz定理・コーシー関数方程式）と明示的に採用した要請（要請O1・O2・M・P）のみを経由して、複数の独立した宇宙論的観測量が記述される。v4.1 以降、Bott 周期は外部定理として引用されず Cayley-Dickson 対合群の内部構造から導出されるため、外部定理への依存はHurwitz定理とコーシー方程式の2つのみに削減された。この構造が本理論の本質的な価値である。
...
*本稿は独自研究の記録（v4.2）である。数値検証コードを含め、独立した検証・批判・議論を歓迎する。*
